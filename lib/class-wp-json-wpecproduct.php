...
class WP_BETAOUT_WpecProduct {
	 ...
	 */
	public function get_posts( $filter = array(), $context = 'view', $type = 'wpsc-product', $page = 1 ) {
		$query = array();
		...
		foreach ( $posts_list as $post ) {
			...
			$response->link_header( 'item', betaout_url( '/posts/' . $post['ID'] ), array( 'title' => $post['post_title'] ) );
			$post_data = $this->prepare_post( $post, $context );
			if ( is_wp_error( $post_data ) ) {
				continue;
			}
                         $product_metadata = get_product_meta( $post['ID'], 'product_metadata', true ); 
                         $product_metadata['price']=get_product_meta($post['ID'], 'price', true );
                         $product_metadata['sku']=get_product_meta($post['ID'], 'sku', true );
                         $product_metadata['stock']=get_product_meta($post['ID'], 'stock', true);
                         $product_metadata['currency']=get_product_meta($post['ID'], 'currency', true);
                        
                         $post_data['productmeta']=$product_metadata;
			$struct[] = $post_data;
		}
	...
	protected function prepare_post( $post, $context = 'view' ) {
		...
		if ( $post['post_date_gmt'] === '0000-00-00 00:00:00' ) {
			...
			$date = WP_BETAOUT_DateTime::createFromFormat( 'Y-m-d H:i:s', $post['post_date'], $timezone );
		...
		if ( $post['post_modified_gmt'] === '0000-00-00 00:00:00' ) {
			...
			$modified = WP_BETAOUT_DateTime::createFromFormat( 'Y-m-d H:i:s', $post['post_modified'], $timezone );
