 ...
 * @package WordPress
 */
class WP_BETAOUT_Server implements WP_BETAOUT_ResponseHandler {
	const METHOD_GET    = 1;
	const METHOD_POST   = 2;
	const METHOD_PUT    = 4;
	...
	public function check_authentication() {
		 ...
		 */
		return apply_filters( 'betaout_authentication_errors', null );
	}
	...
	protected function error_to_response( $error ) {
		...
		$response = new WP_BETAOUT_Response( $data, $status );
	...
	public function serve_request( $path = null ) {
		...
		if ( $result instanceof WP_BETAOUT_ResponseInterface ) {
		 ...
		 */
		$served = apply_filters( 'json_betaout_request', false, $result, $path, $this->method, $this );
	...
	public function prepare_response( $data ) {
		if ( ! defined( 'WP_BETAOUT_SERIALIZE_COMPATIBLE' ) || WP_BETAOUT_SERIALIZE_COMPATIBLE === false ) {
			return $data;
		}
		...
		switch ( gettype( $data ) ) {
			...
			case 'object':
				if ( $data instanceof BetaoutSerializable ) {
					$data = $data->betaoutSerialize();
				} else {
	...
	public function parse_date( $date, $force_utc = false ) {
		...
		return betaout_parse_date( $date, $force_utc );
	}
